...
const safeFetchData = async <T>(url: string, options?: any): Promise<T | null> => {
  try {
...
// Função auxiliar para obter dados do jogo com formatação adicional
export const getGameData = async (appid: number): Promise<ISteamSpyGameData> => {
  const data = await safeFetchData<ISteamSpyGameData>(`${spyRoute}?request=appdetails&appid=${appid}`, {
...
// Função para obter os jogos mais jogados do usuário
...
export const getAllGameData = async (
  ...
  const gameNews = gameNewsResult.status === "fulfilled" ? gameNewsResult.value : null;

  if (!gameData) throw new Error(`Failed to retrieve game data for appId: ${appId}`);
...
export const getRecentlyPlayedGames = async (steamUserId: string): Promise<IRecentlyPlayedRoot> => {
  ...
  if (!data) {
    throw new Error(`Failed to fetch recently played games for user ID: ${steamUserId}`);
  }
...
export const getUserAchievementPaginated = async (
  ...
  const achievementsDataSettled = await Promise.allSettled(
    currentPageGames.map(async (item) => {
      ...
      const data = await safeFetchData<IPlayerStatsRoot>(url, { next: { revalidate: 10000 } });
      if (!data) return null;

      const achievementsData = await getStatsToAchievements(data.playerstats.achievements, item.appid);
      const completedCount = achievementsData.filter((ach) => ach.achieved).length;
  ...
  const achievements = allSettleHandler(achievementsDataSettled) as IAchievementsUser[];
...
export const getTrendingGamesRanked = async (rank = "top100in2weeks"): Promise<ISteamSpyGameData[]> => {
  const data = await safeFetchData<TrendingGamePageType>(`${spyRoute}?request=${rank}`);
  ...
  return allSettleHandler(gameDataSettled).map((result) => result?.gameData) as ISteamSpyGameData[];
};
