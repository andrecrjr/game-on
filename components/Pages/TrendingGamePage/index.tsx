...
const TrendingGamePage = ({ gameRankedData }: Props) => {
  ...
  const currentGames =
    gameRankedData?.slice(indexOfFirstGame, indexOfLastGame) || [];
          ...
          <Card className="border-2 border-primary/20 shadow-lg shadow-primary/10 overflow-hidden backdrop-blur-sm bg-background/80">
            <div className="p-4 border-b border-primary/20 flex justify-between items-center">
              <div className="text-sm text-muted-foreground">
                ...
                {Math.min(indexOfLastGame, totalGames)} of {totalGames} games
              </div>
                  ...
                  currentGames.map((game, index) => {
                    ...
                    return (
                      ...
                      >
                        <TableCell className="text-center">
                          <span
                            className={`inline-flex items-center justify-center w-8 h-8 rounded-full ${isTopThree ? 'bg-accent text-white font-bold' : 'bg-muted/20'}`}
                          >
                            {actualIndex + 1}
                          </span>
                        </TableCell>
