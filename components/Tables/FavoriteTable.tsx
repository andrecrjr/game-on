		...
		return (
			<section className="self-center md:self-auto">
				...
				<img
					...
					className="mt-2"
					src={
						(user?.gamesLibraryData &&
							user?.gamesLibraryData.mostPlayedData.avatarHeader) ||
						''
					}
					loading="eager"
					alt={
						(user?.gamesLibraryData &&
							user?.gamesLibraryData.mostPlayedData.name) ||
						''
					}
					width={200}
					height={60}
				...
				<p className="text-sm text-gray-600" title="Time played in this game">
					...
					{user?.gamesLibraryData &&
						convertTiming(
							user?.gamesLibraryData.mostPlayedTime.playtime_forever,
						)}
				</p>
			</section>
		);
